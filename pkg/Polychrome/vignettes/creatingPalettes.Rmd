...
1. The seed colors usually have a small effect on the overall palette, but they
can change the order.  White and black appear to be exceptions; they tend not
to be included in the palette unless forced.
2. We show below that an upper bound on a good set of colors is likely to be
around 40. it's probably better in the long run to build a palette that you like
...
default of 50000.
4. The range deopends on whether you expect to use a light background (take 
<tt>range = c(10, 60)</tt>), a dark background (take tt>range = c(55, 95)</tt>), 
or both (take tt>range = c(30, 80)</tt>).
5. Unless you really need colors called X1, ..., X37, yiu are probably better
...
The probably surprising result is that the seed colors are actually _not_
included in the final palette. There are two reasons for this.  First, the colors
'black' and 'red' are outside the specified (default) luminance range. So, they
are replaced by the closest randomly generated colors that are inside the range.
But that leads us to the second reason. We specified a small value of <tt>M</tt>.
The algorithm operates by randomly selecting a large set of potential colors in
the desired range, and will eventually assemble the palette from this set.  With
only 1000 candidates, there may not be anything that looks close to pure
...
This paltte is essentially indistinguishable from the first one. However, here is
...
Now, we want colors to be 40 units apart to be reliably distinguisdhable by most
viewers. So, each time we pick a oclor, it should "occupy" a sphere of radius 20
...
The final parameterr we want to look at is <tt>M</tt>, the number of candidate
colors that we search through. In general, we expect the time needed to run the
algorithm to be approximately <tt>N*M</tt>, since we msut search through all
