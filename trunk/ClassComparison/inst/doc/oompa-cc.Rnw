...
%\VignetteDepends{oompaBase,PreProcess,ClassComparison}
...
comnparison problem is to find the genes that are differentially
...
The main functions and classes in the ClassComparion package work
...
area into four regions represnting true positives, false positives,
...
and so various researchers have proposed permuation-based methods that
retain the correaltion structure between genes when trying to estimate
...
The package contains three other methods for finding genes that are
...
\item Linear models (\Rfunction{MultiLinearModel}) can be constructed
  ...
  designs.
\end{enumerate}
A future version of this vignette may include more exmaples of their
