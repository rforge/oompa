aspectHeatmap <- function (x, Rowv = NULL, Colv = if (symm) "Rowv" else NULL, 
    ...
    hExp=1, wExp=1, vlines=NULL, hlines=NULL, lasCol=2, ...) 
{
    ...
    on.exit(par(op))
    if (length(lwid) > 2) {
      widow <- c(1, 0.2, sum(lwid*wExp)-1.2)
    } else {
      widow <- c(1, sum(lwid*wExp)-1)
    }
    if (verbose) {
        cat("layout: widths = ", lwid, ", heights = ", lhei, 
            ", hExp = ", hExp, ", wExp = ", wExp, "; lmat=\n")
        print(lmat)
        print(widow)
    }
    layout(lmat, widths = widow, heights = lhei*hExp, respect = TRUE)
    if (!missing(RowSideColors)) {
