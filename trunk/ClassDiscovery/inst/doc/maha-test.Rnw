%\VignetteIndexEntry{OOMPA Mahalnobis Distance}
%\VignetteKeywords{OOMPA, PCA, Mahalnobis Distance, Outliers}
...
\title{PCA, Mahalnobis Distance, and Outliers}
...
we see that evne out at the level of $20$ components, this sample
remains an outlier.
<<redmaha>>=
maha20 <- mahalanobisQC(spca, 20)
@ 
