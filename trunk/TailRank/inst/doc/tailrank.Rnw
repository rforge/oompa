...
alternative method for dicovering poitential biomarkers in large data
...
The \Rpackage{TailRank} package comes with some sample data that we
...
<<summary>>=
clinical.info$Status <- ordered(clinical.info$Status, levels=c("N", "T", "L"))
...
test into a logival vector that selects these significant genes.
