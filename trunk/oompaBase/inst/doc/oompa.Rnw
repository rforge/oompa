...
rcitical (but invisible to the user) feature of the
\Rpackage{oompaBase} package is that it defines a 
\Robject{class union} allowing you to use ``numeric'' or ``NULL''
objects in the design of an S4 class.  More intersting user-visibel
features include alternative color schemes and vectorized matrix
operations to speed the computation of row-by-row mweans, variances,
and t-tests. 
...
@ 
Next, we shift the mean for te first $500$ ``genes'' (rows).
<<dat>>=
...
used0 <- proc.time() - a0
@ 
For comparions purposes, we perfom the same computation using
\Rfunction{apply}.
<<checkApply>>=
a1 <- proc.time()
mm <- apply(dat, 1, mean)
